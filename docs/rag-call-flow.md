...
- **Qdrant** : stocke les embeddings générés via le modèle d’embedding Ollama (`spring.ai.ollama.embedding.model`).
...
sequenceDiagram
    ...
    RagService->>Ollama: embed(content)
    Ollama-->>RagService: embedding vector
...
- **rag-service** (`RagService.answer/streamAnswer`) :
  ...
  - Appel LLM via `ChatClient` Ollama (`spring.ai.ollama.chat.model` + `OllamaOptions.withModel`).
...
sequenceDiagram
    ...
    participant RagService
    participant Qdrant
    participant Ollama as Ollama (chat)
    ...
    RagService->>Ollama: chat(system+user prompt with context)
    Ollama-->>RagService: answer/stream chunks
    RagService-->>Gateway: answer/stream
    Gateway-->>Client: answer/stream
```

## 4. Points de configuration clés
- Modèle de chat : `spring.ai.ollama.chat.model` (exposé en env `OLLAMA_MODEL` dans `docker-compose.yml`).
- Modèle d’embedding : `spring.ai.ollama.embedding.model` (exposé en env `OLLAMA_EMBEDDING_MODEL`, par défaut même valeur que le modèle de chat).
...
sequenceDiagram
    ...
    Compose->>RagApp: set env OLLAMA_MODEL / OLLAMA_EMBEDDING_MODEL\nset QDRANT_HOST/PORT
    RagApp->>Ollama: configure chat+embedding models
